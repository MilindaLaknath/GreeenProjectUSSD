...
//        $connection = mysqli_connect("localhost", "root", "123", "sessiondb");
        ...
        $sql_sessions = "INSERT INTO `sessions` (`sessionsid`, `tel`, `menu`, `pg`, `created_at`,`others`,`longitude`,`latitude`) VALUES 
...
//        mysqli_close($connection);
    ...
    public function getGcolectors($type, $latitude, $longitude) {
         ...
         */
        if ($type === "All") {
            $serch_qry = "SELECT gcid,organization,city,( 6371 * acos( cos( radians($latitude) ) * cos( radians( lat )) * cos( radians( lng) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( lat ) ) ) ) AS distance FROM gbcollector HAVING distance < 10 ORDER BY distance  LIMIT 0 , 5";
        } else {
            $serch_qry = "SELECT gcid,organization,city,( 6371 * acos( cos( radians($latitude) ) * cos( radians( lat )) * cos( radians( lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( lat ) ) ) ) AS distance FROM gbcollector, collector_types ct WHERE gcid=ct.collector_id AND ct.type_id='" . $type . "' HAVING distance < 10 ORDER BY distance  LIMIT 0 , 5";
        }
...
//        $rand_pin = hexdec($uniqid);
        ...
        $sehqry = "SELECT gcid,type FROM tmp_save WHERE sessid='" . $sesId . "' AND countnum='" . $resp_no . "'";
        $ressult_gcid = mysqli_query($connection, $sehqry);
        while ($row_gcid = mysqli_fetch_assoc($ressult_gcid)) {
            $gcid = $row_gcid["gcid"];
            $gctype = $row_gcid["type"];
        }
        $serch_qry = "SELECT * FROM gbcollector WHERE gcid='" . $gcid . "'";
        $reslt = mysqli_query($connection, $serch_qry);
        while ($row = mysqli_fetch_assoc($reslt)) {
            $msg = "Name : " . $row["organization"] . "\nCity : " . $row["city"] . "\nPin : " . $rand_pin;       //"\nNo : " . $row["number"] .
        }

        $pinqry = "INSERT INTO individual(sessionid,gtype,assigned,collected,pin) VALUES('" . $sesId . "','".$gctype."','" . $gcid . "',0,'" . $rand_pin . "')";
        mysqli_query($connection, $pinqry);

        $del_tmp = "DELETE FROM tmp_save WHERE sessid='" . $sesId . "'";
    ...
    function setArtoStr($res_ar, $sesId,$gctype) {
        ...
        foreach ($res_ar as $value) {
            foreach ($value as $x => $x_value) {
                ...
                $tmp_qry = "INSERT INTO tmp_save(sessid,countnum,gcid,type) VALUES('" . $sesId . "','" . $x . "','" . $x_value[0] . "','".$gctype."')";
